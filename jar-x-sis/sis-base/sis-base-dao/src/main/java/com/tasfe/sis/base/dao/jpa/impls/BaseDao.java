...
public class BaseDao<T> {
     ...
     * @return 是否存在
     */
    public boolean contains(final T model) {
     ...
     * @param model 实体
     */
    public void detach(final T model) {
     ...
     *                   失败抛出异常
     */
    public void deleteById(final Class<T> modelClass, final Serializable id) {
     ...
     *                  失败会抛出异常
     */
    public void saveOrUpdateAll(final List<T> modelList) {
     ...
     */
    @Transactional(readOnly = true)
    public PageResults<T> getListByPageAndQuery(Integer currentPageNumber, Integer pageSize, Query query) throws Exception {
        //获得符合条件的总数目
        ...
        //查看是否要分页
        if (currentPageNumber > 0 && pageSize > 0) {
            typedQuery.setFirstResult((currentPageNumber - 1) * pageSize).setMaxResults(pageSize);
        }
     ...
     */
    @Transactional(readOnly = true)
    public int getCountByQuery(final Query query) {
        query.selectCount();
     ...
     * @return 受影响的行数
     */
    public int executeSql(String sql, final Object... values) {
     ...
     */
    @Transactional(readOnly = true)
    public PageResults<Object> getListByPageAndJpql(Integer currentPageNumber, Integer pageSize, final String jpql, final Object... values) {
        //参数验证
        ...
        //查看是否要分页
        if (currentPageNumber > 0 && pageSize > 0) {
            query.setFirstResult((currentPageNumber - 1) * pageSize).setMaxResults(pageSize);
        }
