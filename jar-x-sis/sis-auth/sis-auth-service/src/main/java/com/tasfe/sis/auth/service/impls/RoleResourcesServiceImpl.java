...
public class RoleResourcesServiceImpl implements RoleResourcesService {
    ...
    @Override
    public List<Resources> getResourcesWithRoleId(Long roleId) throws Exception {
        RoleResources record = new RoleResources();
        record.setRoleId(roleId);
        List<RoleResources> list = crudTemplate.find(record, Criteria.from(RoleResources.class));
        List<Resources> resList = new ArrayList<Resources>();
        for(RoleResources roleResources:list) {
            Resources record1 = new Resources();
            record1.setId(roleResources.getResourceId());
            List<Resources> resources = crudTemplate.find(record1, Criteria.from(Resources.class));
            if(!CollectionUtils.isEmpty(resources)) {
                resList.add(resources.get(0));
            }
        }
        return resList;
    }
